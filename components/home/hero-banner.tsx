...
import { HomeBanner } from '@/lib/api/home-settings';
...
export function HeroBanner({ slides: propSlides }: HeroBannerProps) {
  // Fallback to defaults if no slides provided
  ...
  return (
    <section className="relative w-full h-[500px] md:h-[600px] lg:h-[716px] bg-[#26C6D0] rounded-[5px] overflow-hidden">
      <div className="absolute inset-0 transition-opacity duration-500">
        <Image
          src={currentSlideData.image || ASSETS.heroBanner}
      ...
      <div className="absolute left-8 md:left-24 lg:left-[193px] top-20 md:top-28 lg:top-[129px] w-[90%] md:w-[500px] lg:w-[439px] text-white z-10">
        ...
        <LinkButton
          href={currentSlideData.link || "/products"}
          variant="white"
      ...
      <div className="absolute bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 flex rounded-xl overflow-hidden z-20">
        {slides.map((_, index) => (
          <button
            ...
            className={`w-12 md:w-[62px] h-2 md:h-2.5 transition-colors ${index === currentSlide ? 'bg-white' : 'bg-white/50'
              }`}
